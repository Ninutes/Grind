...
from pycolorise.colors import Green, BrightYellow, BrightBlue, Red
...
class Grind(commands.Bot):
    def __init__(
        ...
        **kwargs,
        ...
        self.mysession = tls_client.Session(
            client_identifier="firefox_111", random_tls_extension_order=True
        )
        ...
        for foldername, _, filenames in os.walk(cog_dir):
            for filename in filenames:
                if filename.endswith(".py"):
                    ...
                    cog_name = os.path.relpath(cog_path, cog_dir)[:-3].replace(
                        os.path.sep, "."
                    )
                    try:
    ...
    def auth(self, token):
        ...
        refer_oauth = (
            self.myclient.get(oauth_reqstr).text.split('<a href="')[1].split('">')[0]
        )
        ...
        if response.status_code == 200:
            if "location" in response.text:
                ...
                hosturi = (
                    locauri.replace("https://", "").replace("http://", "").split("/")[0]
                )
                headers = {
                ...
                if res2.status_code in (302, 307):
                    ...
                    print(
                        f"(-) Failed to add token to oauth | {res2.text}, {res2.status_code}"
                    )
    ...
    async def on_ready(self):
        ...
        print(
            f"\033[1m[ \033[0m{BrightYellow(self.user.name)}\033[1m ]\033[0m is \033[1m{BrightBlue('Online')}\033[0m "
        )
    ...
    async def on_message(self, message: selfcord.Message) -> None:
        if (
            message.author.id in GLOBAL.get_value("allowedID")
            or message.author.id == GLOBAL.owoID
            or message.author.id == GLOBAL.reactionID
    ...
    async def log_on_ready(self):
        ...
        embed = selfcord.Embed(
            color=selfcord.Color.light_gray(), timestamp=datetime.now()
        )
        embed.set_thumbnail(
            url="https://cdn.discordapp.com/emojis/1230304058405814332.gif"
        )
        embed.add_field(
            name="Self-Bot", value=f"connected to **{servers} guilds**", inline=False
        )
