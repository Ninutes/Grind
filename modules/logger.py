...
async def _webhook(
    msg: selfcord.Message = None,
    content: str = None,
    embed: Embed = None,
    type: str = "log",
):
    url = (
        GLOBAL.get_value("webhook.URL")
        if type == "log"
        else GLOBAL.get_value("webhook.captchaURL")
    )
    async with aiohttp.ClientSession() as session:
        ...
        await webhook.send(
            ...
            avatar_url=(
                msg.author.avatar.url if msg else GLOBAL.get_value("user.avatarURL")
            ),
            embed=embed,
        )
...
async def send_tele(content):
    try:
        post(
            url=f"https://api.telegram.org/{Auth.TELEAPI}/sendMessage?chat_id={GLOBAL.get_value('tele.chatID')}&text={content}&parse_mode=MarkdownV2"
        )
    except Exception as e:
        await LOG.failure(e)
...
class Log:
    ...
    async def captcha(
        self, message: selfcord.Message, key: str, fail: Optional[str] = None
    ):
        ...
        if balance:
            embed.set_footer(
                ...
                icon_url="https://cdn.discordapp.com/emojis/1179649531927863397.webp",
            )
        ...
        await _webhook(content=f"<@{self.wb_ping}>", embed=embed, type="captcha")

    async def captcha_failed(self, reason: str):
        embed = Embed(description=reason, color=selfcord.Color.red())
        await _webhook(embed=embed, type="captcha")

    async def captcha_info(self, reason: str):
        embed = Embed(description=reason, color=selfcord.Color.dark_gray())
        await _webhook(embed=embed, type="captcha")
    ...
    async def success(self, reason: str):
        color = selfcord.Color.green()
        embed = Embed(
            description=reason, timestamp=datetime.datetime.now(), color=color
        )
        await _webhook(
            embed=embed,
        )
    ...
    async def log_config(self):
        ...
        if data:
            channel = (
                GLOBAL.g_channel.id
                if GLOBAL.g_channel is not None
                else GLOBAL.get_value("channelID")
            )
        ...
        config = f"`{pray}` pray\n`{gem}` gem\n`{exp}` exp\n`{sleep}` sleep\n`{solve}` solve\n`{custom}` custom text"
        embed = Embed(description=f"**CONFIG**\n{config}", color=selfcord.Color.blue())
        await _webhook(content=f"> **I am here : [ <#{channel}> ] **", embed=embed)

    async def log_avatar(self, user: selfcord.Member):
        embed = Embed(color=selfcord.Color.random())
        embed.set_author(name=f"{user.name} avatar")
        embed.set_image(url=user.display_avatar.url)
        await _webhook(embed=embed)
