...
class Gems(commands.Cog):
    ...
    def is_myhunt(self, message):
        return (
            GLOBAL.get_value("gem")
            and message.channel == GLOBAL.g_channel
            and message.author.id == self.owoID
    ...
    async def set_gem(self, ctx: commands.Context, use: int = None):
        ...
        await LOG.success("**GEMS MODE** set to: %s" % GLOBAL.get_value("gem"))
    ...
    async def detect_gems(self, message: selfcord.Message):
        if self.bot.user.display_name not in message.content:
            return
        current_gems = [
    ...
    async def use_gems(self, target=[1, 3, 4]):
        ...
        async for msg in GLOBAL.g_channel.history(limit=5):
            inv = []
            if (
                msg.author.id == self.owoID
                and self.bot.user.display_name in msg.content
                and "Inventory" in msg.content
            ):
        ...
        if not GLOBAL.is_captcha and use:
            async for msg in GLOBAL.g_channel.history(limit=5):
                if (
                    "you activated" in msg.content.lower()
                    and "gem" in msg.content.lower()
                    and "you already have an active" not in msg.content
                    and self.bot.user.display_name in msg.content
                ):
                    return
