...
class Captcha(commands.Cog):
    ...
    def cog_check(self, ctx: commands.Context):
        return (
            ctx.author.id in GLOBAL.get_value("allowedID") or ctx.author.id == ctx.me.id
        )
    ...
    async def detect_captcha(self, m: selfcord.Message, key: str, text: str):
        ...
        if not dm_channel:
            if any(n in text for n in name) and key in text:
                return True
            return False
        else:
            if key in text and "verified" not in text:
                return True
            elif "3/3" in text:
    ...
    async def get_captcha(self, message: selfcord.Message):
        ...
        async for msg in message.channel.history(limit=10):
            ...
            if "link" in text and any(n in text for n in name):
                return await self.solver_link(message)
            if msg.attachments and "captcha" in text and any(n in text for n in name):
                self.captcha_image = b64encode(await msg.attachments[0].read()).decode(
                    "utf-8"
                )
                self.captcha_length = msg.content[msg.content.find("letter word") - 2]
                if GLOBAL.get_value("autosolve"):
                    return await self.get_result(
                        msg, self.captcha_image, self.captcha_length
                    )
                return await LOG.captcha(msg, "detected")
            if (
                "captcha" in text
                and any(string in text for string in ["1/5", "2/5", "3/5"])
                and any(n in text for n in name)
            ):
    ...
    async def get_result(self, message: selfcord.Message, image, length):
        ...
        while True:
            ...
            if time() - solve_time > 180:
                await LOG.captcha_failed(
                    "180's have been left, probably captcha unsolvable"
                )
    ...
    async def send_result(self, message, result):
        ...
        async for msg in self.owoDM.history(limit=1):
            if "verified" in msg.content:
                ...
                if "2/3" not in msg.content:
                    await asyncio.sleep(10)
                    return await self.get_result(
                        message, self.captcha_image, self.captcha_length
                    )
                elif "2/3" in msg.content:
                    return await LOG.captcha_failed(
                        "Auto solve failed, after 2 attempts."
                    )
            elif "banned" in msg.content:
                ban = re.search(self.regex_ban, msg.content)
                ban_time = int(ban.group(1))
                ban_time = (
                    datetime.now() + timedelta(hours=ban_time) + timedelta(hours=7)
                )
                ban_time = ban_time.strftime("%d %B %Y %H:%M")
                GLOBAL.is_captcha = True
                return await LOG.captcha_failed(
                    f"You have been banned until **{ban_time}**"
                )
            return await LOG.info("OwO didn't response")
        ...
        with concurrent.futures.ThreadPoolExecutor() as pool:
            try:
                result = await loop.run_in_executor(
                    pool,
                    lambda: TwoCaptcha(Auth.APIKEY).normal(
                        image,
                        numeric=2,
                        minLen=length,
                        maxLen=length,
        ...
        try:
            with concurrent.futures.ThreadPoolExecutor() as pool:
                captcha = await loop.run_in_executor(
                    pool, lambda: TwoCaptcha(Auth.APIKEY)
                )
    ...
    async def captcha_count(self, ctx: commands.Context, amount: int, limit: int):
        await self._delete_msg(ctx)
        self.owoDM = self.bot.get_user(self.owoID).dm_channel
        total = 0
        if amount >= 1:
            async for message in self.owoDM.history(limit=limit):
                if (
                    message.content.count("cross_box") == amount
            ...
            async for message in self.owoDM.history(limit=limit):
                if (
                    message.content.count("blank_box") == 3
                    and "captcha" in message.content
                ):
                    total += 1
                if (
    ...
    async def on_message(self, message: selfcord.Message):
        ...
        if message.author.id == self.owoID:
            teks = re.sub(r"[\u200B-\u200D\uFEFF]", "", message.content)
            if any(word in teks for word in ["captcha", "real human", "link"]):
                self.owoDM = self.bot.get_user(self.owoID).dm_channel
                detect = await self.detect_captcha(message, "⚠️", teks)
                if detect:
                    GLOBAL.is_captcha = True
