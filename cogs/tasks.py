...
class Tasks(commands.Cog):
    ...
    async def on_message(self, message: selfcord.Message):
        ...
        if not message.channel.id == GLOBAL.get_value("channelID"):
            return
        if self.sleep or GLOBAL.is_captcha:
            return
        if message.author.id == GLOBAL.get_value(
            "user.ID"
        ) and message.content.startswith(f"{GLOBAL.get_value('owoprefix')}h"):
            try:
                await self.bot.wait_for(
                    "message",
                    timeout=30,
                    check=lambda message: message.author.id == GLOBAL.owoID
                    and message.channel.id == GLOBAL.get_value("channelID"),
                )
    ...
    async def random_hunt_battle(self) -> None:
        ...
        if time() - self.ohb_time >= randrange(14, 17) and not GLOBAL.is_captcha:
    ...
    async def pray(self) -> None:
        ...
        if not prayON["enable"]:
            return
        if time() - self.pray_time >= randrange(300, 320) and not GLOBAL.is_captcha:
            await GLOBAL.g_channel.typing()
            await asyncio.sleep(random.randint(1, delay))
    ...
    async def random_exp(self):
        ...
        if exp_mode:
            if (
                ...
                await GLOBAL.g_channel.send(
                    random_quotes["quoteText"] + " - " + random_quotes["quoteAuthor"]
                )
            self.exp_time = time()
    ...
    async def random_cmd(self):
        ...
        if time() - self.random_cmd_time == time():
            ...
            return
        if (
            time() - self.random_cmd_time >= random.randint(120, 1200)
            and not GLOBAL.is_captcha
        ):
            await GLOBAL.g_channel.typing()
