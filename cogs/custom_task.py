...
class CustomTask(commands.Cog):
    ...
    def cog_check(self, ctx: commands.Context):
        return ctx.author.id == GLOBAL.get_value(
            "user.ID"
    ...
    async def counter_time(self):
        ...
        return await LOG.info(
            f"`⛔` stopping custom tasks after running in **{hours}H {minutes}M {seconds}S**"
        )

    @commands.command(name="custom")
    async def custom_tasks(
        self, ctx: commands.Context, task_number: int, key: str, *, value: str
    ):
        await self._delete_msg(ctx)
        data = GLOBAL.get_all_data("tasks")
        if task_number not in data:
            return await LOG.failure(
                f"{task_number} is not a valid task. available tasks:{[task for task in data]}"
            )
        if key not in data[task_number]:
            return await LOG.failure(
                f"{key} is not a valid key. available keys:{[key for key in data[task_number]]}"
            )
